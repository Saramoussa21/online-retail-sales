...
class RetailDataCleaner(DataCleaner):
    ...
    def _clean_quantity(self, quantity: str) -> int:
        ...
        try:
            ...
            if qty_value < 0:
                self.logger.info(f"Converting negative quantity {qty_value} to positive (return transaction)")
...
class DataCleaningPipeline:
    ...
    def get_summary_metrics(self) -> Dict[str, Any]:
        """Get summary of cleaning metrics"""
        cleaner_metrics = self.cleaner.get_metrics()
        
        return {
            'total_processed': self.total_processed,
            'total_cleaned': self.total_cleaned,
            'total_rejected': self.total_rejected,
            'cleaning_rate': (self.total_cleaned / self.total_processed * 100) if self.total_processed > 0 else 0,
            'duplicates_detected': self.duplicate_handler.get_duplicate_count(),
            'outliers_detected': self.outlier_detector.outlier_count,
            'missing_values_handled': self.missing_value_handler.missing_count,
            'cleaning_rules_applied': cleaner_metrics.cleaning_rules_applied,
            'validation_errors': cleaner_metrics.validation_errors
        }
    
    def log_summary(self) -> None:
        """Log cleaning summary"""
        metrics = self.get_summary_metrics()
        self.logger.info("Data cleaning pipeline completed", **metrics)
